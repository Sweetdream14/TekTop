...
class MailController extends Controller
{
    public function send_mail(){
        $to_name = "TekTop";
        $to_email = "tplinkvietnam89@gmail.com";
        
        $data = array("name"=>"TekTop","body"=>"TekTop"); 
            
        Mail::send('pages.send_mail',$data,function($message) use ($to_name,$to_email){
            $message->to($to_email)->subject('TepTop - Thiết bị công nghệ');
	...
	public function send_coupon_vip($coupon_time,$coupon_condition,$coupon_number,$coupon_code){
		//get customer
		$customer_normal = Customer::where('customer_normal',0)->get();
        

		
        if(count($customer_normal) != 0 ){
            $coupon = Coupon::where('coupon_code',$coupon_code)->first();
        $start_coupon = $coupon->coupon_date_start;
        $end_coupon = $coupon->coupon_date_end;
        ...
        foreach($customer_normal as $vip){
    ...
    public function reset_new_pass(Request $request){
    	...
    	if($count>0){
            ...
            $reset->save();
            return redirect('login-checkout')->with('message','Mật khẩu đã được, vui lòng quay lại trang đăng nhập');
    ...
    public function quen_mat_khau(Request $request){
    	...
    	return view('pages.checkout.forget_pass')->with('category',$cate_product)->with('brand',$brand_product)->with('all_product',$all_product)->with('all_product',$all_product)->with('meta_desc',$meta_desc)->with('meta_keywords',$meta_keywords)->with('meta_title', $meta_title)->with('url_canonical', $url_canonical)->with('slider',$slider)->with('category_post',$category_post)->with('cate_pro_tabs',$cate_pro_tabs);
    }
