...
class CheckoutController extends Controller
{
    ...
    public function confirm_order(Request $request){
      ...
      //lay shipping
    if(Session::get('free')==true){
        $free = Session::get('free').'k';
    }else{
        $free = '25k';
    }
    ...
    public function checkout(Request $request){
       ...
       $slider = Slider::orderBy('slider_id','desc')->where('slider_status','0')->take(4)->get();
    	...
    	return view('pages.checkout.show_checkout')->with('category',$cate_product)->with('brand',$brand_product)->with('meta_desc',$meta_desc)->with('meta_keywords',$meta_keywords)->with('meta_title',$meta_title)->with('url_canonical',$url_canonical)->with('city',$city)->with('slider',$slider)->with('category_post',$category_post)->with('min_price',$min_price)->with('max_price',$max_price)->with('edit_introductions',$edit_introductions);
    }
