		...
		<div class="col-md-12" >
			...
			<div class="row" style="background: cadetblue;padding: 15px">
				...
				<div class="col-md-6" style="text-align: center;color: #fff;font-weight: bold;font-size: 30px">
					<h4 style="margin:0">THIẾT BỊ CÔNG NGHỆ - TEKTOP</h4>
					<h6 style="margin:0">GIÁ CẢ PHẢI CHĂNG - THIẾT BỊ CHÍNH HÀNG</h5>
				</div>

				<div class="col-md-6 logo"  style="color: #fff">
					<p>Chào bạn :<strong style="color: #000;">{{$shipping_array['customer_name']}}</strong></p>
				</div>
				
				<div class="col-md-12">
					<p style="color:#fff;font-size: 17px;">Bạn {{$shipping_array['shipping_name']}} đã đăng ký dịch vụ tại shop với thông tin như sau:</p>
					...
					<p>Phí ship hàng : <strong style="text-transform: uppercase;color:#fff">{{$shipping_array['free']}}</strong></p>
					...
					<table class="table table-striped" style="border:1px solid; width: 500px;" >
						<thead>
							<tr>
								<th style="width: 170px">Sản phẩm</th>
								<th style="width: 130px">Giá tiền</th>
						...
						<tbody>
							...
							<tr>
								<td style="width: 170px">{{$cart['product_name']}}</td>
								<td style="width: 130px;text-align: center">{{number_format($cart['product_price'],0,',','.')}} VNĐ</td>
								<td style="width: 60px; text-align: center">{{$cart['product_qty']}}</td>
								<td style="width: 130px;text-align: center">{{number_format($cart['product_price'],0,',','.')}} VNĐ</td>
							...
							<tr>
								...
								<td colspan="3" align="left">Tổng tiền thanh toán: </td>
								<td style="width: 100px;text-align: center">{{number_format($total,0,',','.')}} VNĐ</td>
							</tr>
				...
				<p style="color:#fff">Mọi chi tiết xin liên hệ website tại: <a target="_blank" href="http://tektop.uk">Shop</a>, hoặc liên hệ qua số hotline: 0984.880.966.  cảm ơn quý khách đã đặt hàng shop chúng tôi.</p>
